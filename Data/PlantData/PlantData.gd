...
# need to manually add the array to be filled for any new seed types
var seedPacks: Array
...
#PlantStage, #terms in pack, #section in pack, all seeds introduced?
var seeds_animals_info = [0, 0, 1, false] 
...
func _ready():
	# Goes through dictionary and fills all arrays
	makeSeedPack() 
...
func assessPlant(seedsToPlant): 
	...
	# if seedsToPlant is in smallSeedPacks array:
	if str(seedsToPlant) in smallSeedPacks:
		# Then for each term in currentSeedPack:
		for seedWords in currentSeedPack.size():
			print(currentSeedPack)
			# add term if 
